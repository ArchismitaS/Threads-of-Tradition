...
function setLessonsStatus(message, isError = false) {
  const el = document.querySelector("[data-lessons-status]");
  if (!el) return;
...
function syncLessons(nextState) {
  ...
  const toggleLesson = async (id, button, checkbox) => {
    ...
    try {
      ...
      setLessonsStatus("Progress saved.");
    } catch (error) {
      console.error(error);
      setLessonsStatus("Could not save lesson progress. Make sure the backend server is running.", true);
  ...
  lessonCards.forEach((card) => {
    ...
    if (checkbox) {
      ...
      checkbox.onchange = async () => {
        await toggleLesson(id, button, checkbox);
      };
    }
